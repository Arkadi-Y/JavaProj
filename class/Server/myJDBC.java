...
public class myJDBC implements myJDBCInt {
    ...
    public String getQuery(String column, String table, String where, String ID) {
        ...
        try {
            ...
            ResultSet resultSet = statement
                    .executeQuery("select " + column + " from " + table + " where " + where + "=" + ID);
            while (resultSet.next()) {
    ...
    public boolean validateLogin(String username, String userpassword) {
        try {
            Connection connection = DriverManager.getConnection(sqlUrl, user, password);
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from employeelogin WHERE UserName ='" + username
    ...
    public int findPerson(String name, String phone) {
        try {
            ...
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement
                    .executeQuery("select * from person WHERE name ='" + name + "' AND phone ='" + phone + "'");
            while (resultSet.next()) {
    ...
    public void insertTicket(Ticket t) {
        try {
            Connection connection = DriverManager.getConnection(sqlUrl, user, password);
            Statement statement = connection.createStatement();
            statement.executeUpdate("INSERT INTO tickets VALUES (" + t.getTicketNum() + ",'" + t.getDescription()
                    + "','" + t.getStatus() + "'," + t.getId() + ")");
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    public void newPerson(Person p) {
        try {
            ...
            if (findPerson(p.getName(), p.getPhone()) > -1)
                ...
                statement.executeUpdate("INSERT INTO person VALUES (" + p.getId() + ",'" + p.getName() + "','"
        ...
        try {
            ...
            if (Id > -1)
                ...
                statement.executeUpdate(
                        "INSERT INTO person VALUES (" + id + ",'" + name + "','" + phone + "','" + mail + "')");
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    }
